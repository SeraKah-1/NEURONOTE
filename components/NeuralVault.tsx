...
const NeuralVault: React.FC<NeuralVaultProps> = ({ onSelectNote, onImportCloud }) => {
  ...
  const filteredNotes = notes.filter(n => {
      const matchesSearch = n.topic.toLowerCase().includes(search.toLowerCase()) || n.content.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="flex flex-col h-full overflow-hidden">
        ...
        {allTags.length > 0 && (
            <div className="px-4 py-2 border-b border-white/5 flex gap-2 overflow-x-auto custom-scrollbar bg-gray-900/30 shrink-0">
                <div className="flex items-center gap-1 text-[10px] font-bold text-gray-500 uppercase mr-2">
                    <Filter size={10} /> Tags:
                </div>
                <button 
                    onClick={() => setActiveTag(null)}
                    className={`px-2 py-0.5 rounded text-[10px] border transition-colors ${!activeTag ? 'bg-white/10 border-white/20 text-white' : 'border-transparent text-gray-600 hover:text-gray-400'}`}
                >
                    ALL
                </button>
                {allTags.map(tag => (
            ...
            {loading ? (
                ...
                <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" : "flex flex-col gap-2"}>
                    {filteredNotes.map(note => (
                        <div 
                            key={note.id}
                            onClick={() => note._status === 'cloud' ? onImportCloud(note) : onSelectNote(note)}
                            className={`
                        ...
                        >
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    {note._status === 'cloud' && (
                                        <button className="p-1.5 hover:bg-white/10 rounded text-cyan-400" title="Import"><Download size={14}/></button>
                                    )}
                            ...
                            <p className="text-xs text-gray-500 line-clamp-2 mb-3 h-8">{note.content.substring(0, 150).replace(/[#*`]/g, '')}</p>
