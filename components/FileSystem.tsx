...
const FileSystem: React.FC<FileSystemProps> = ({ onSelectNote, activeNoteId }) => {
  ...
  const handleRenameNote = (id: string, current: string) => {
          ...
          >
              ...
              {activeMenuId === note.id && (
                  <div ref={menuRef} className="absolute right-2 z-50 w-32 bg-[var(--ui-surface)] border border-[var(--ui-border)] shadow-xl rounded-lg overflow-hidden animate-fade-in flex flex-col py-1">
                      ...
                      {note._status === 'local' && <button onClick={(e) => { e.stopPropagation(); storage.uploadNoteToCloud(note).then(() => {alert("Uploaded!"); refreshData();}); setActiveMenuId(null); }} className="px-3 py-2 text-left text-[10px] hover:bg-[var(--ui-bg)] flex items-center gap-2 text-cyan-500"><UploadCloud size={10}/> Upload Cloud</button>}
