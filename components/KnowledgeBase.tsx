...
import { 
  ...
  FolderOpen, Link as LinkIcon, Lock, X, BrainCircuit, Shuffle, Play, Cpu, Layers, UploadCloud, FileUp, Grid, List as ListIcon, File
} from 'lucide-react';
import { StorageService } from '../services/storageService';
import { DriveService } from '../services/driveService';
import { KnowledgeSource, KnowledgeFile, LibraryMaterial } from '../types';
...
const KnowledgeBase: React.FC = () => {
  ...
  const loadData = async () => {
    if (isCloudMode) {
      try {
        ...
        // Mock/Local loading simplified for this view
        const localSources = storage.getKnowledgeSources();
        // ... (Local logic similar to before, adapting to Unified Format for display)
    }
  };

  const handleLocalUploadClick = () => fileInputRef.current?.click();
  ...
  const handleDeleteItem = async (id: string, e: React.MouseEvent) => {
      ...
      if(confirm("Remove this material?")) {
          if (isCloudMode) await storage.deleteLibraryMaterial(id);
          loadData();
      }
  };
      ...
      <div className="p-6 border-b border-[var(--ui-border)] bg-[var(--ui-surface)] shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
         <div>
            ...
            <p className="text-[var(--ui-text-muted)] text-sm mt-1">
               {isCloudMode ? "Connected to Supabase Library (God Mode)" : "Local Storage Mode"}
            </p>
         </div>
         ...
         <div className="flex items-center gap-3">
             ...
             <button 
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--ui-primary)] hover:opacity-90 text-white text-sm font-bold shadow-lg transition-all"
             >
                {isProcessing ? <RefreshCw className="animate-spin" size={16}/> : <UploadCloud size={16}/>}
                <span>Upload</span>
             </button>
             <input type="file" ref={fileInputRef} onChange={handleLocalFileChange} className="hidden" multiple />
         </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          {cloudMaterials.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-[var(--ui-text-muted)] border-2 border-dashed border-[var(--ui-border)] rounded-2xl">
                  ...
                  <p>Library is empty. Upload PDFs or Text to begin.</p>
              </div>
          ) : viewMode === 'grid' ? (
