...
const App: React.FC = () => {
  ...
  const [settingsTab, setSettingsTab] = useState<'keys' | 'storage'>('keys'); 
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (focusMode && e.key === 'Escape' && !showPalette) {
         setFocusMode(false);
      }
    };
  ...
  const handleAutoStructure = async () => {
    ...
    try {
        let struct = '';
        if (config.provider === AIProvider.GEMINI) {
           struct = await generateDetailedStructure(config, noteData.topic);
        } else {
           if(!config.groqApiKey) {
              alert("Groq Key needed for Groq Auto-Draft.");
              setIsStructLoading(false);
              return;
           }
           struct = await generateDetailedStructureGroq(config, noteData.topic);
        }
  ...
  return (
    <div className="min-h-screen bg-neuro-bg text-neuro-text flex flex-col md:flex-row font-sans overflow-hidden selection:bg-neuro-primary/30 selection:text-white">
      ...
      {/* --- SIDEBAR: CONTROL CENTER (HIDDEN IN FOCUS MODE) --- */}
      <aside className={`w-full md:w-[280px] lg:w-[320px] glass-panel p-5 flex flex-col shrink-0 z-30 h-screen overflow-hidden shadow-2xl border-r border-white/5 transition-all duration-300 ${focusMode ? 'hidden md:hidden' : 'block'}`}>
      ...
      <main className="flex-1 relative h-screen overflow-hidden flex flex-col bg-neuro-bg">
        ...
        {focusMode && (
           <button 
             ...
             className="absolute top-4 right-4 z-50 p-3 bg-gray-900/80 hover:bg-gray-800 text-white rounded-full border border-gray-700 shadow-xl backdrop-blur-md transition-all hover:scale-110 group"
             title="Exit Focus Mode (ESC)"
           >
             <Minimize2 size={20} className="group-hover:text-neuro-primary transition-colors"/>
           </button>
        )}
          ...
          {appState.currentView !== AppView.GRAPH && !focusMode && (
            <div className="flex justify-between items-start mb-6 shrink-0">
               ...
               <div className="flex items-center gap-3">
                 <button onClick={() => setShowPalette(true)} className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-gray-900 border border-gray-700 rounded-lg text-xs text-gray-400 hover:text-white hover:border-gray-500 transition-all">
                      ...
                      {isStructLoading && (
                          <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm rounded-xl">
                             <div className="text-center">
                                 <RefreshCw size={24} className="animate-spin text-neuro-primary mx-auto mb-2"/>
                                 <span className="text-xs text-white font-mono uppercase tracking-widest">Generating Structure...</span>
                             </div>
                          </div>
                      )}
                    </div>
